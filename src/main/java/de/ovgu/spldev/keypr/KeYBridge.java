...
public class KeYBridge {
    ...
    static Proof proveContract(File file, Mode mode, OptimizationStrategy optimizationStrategy, Map<String, String> strategyProperties, Signature.Method signature, boolean isAbstractProof, boolean useAbstractContracts) {
        ...
        return keYBridge.proveContract(contract, isAbstractProof, useAbstractContracts);
    }
    ...
    private void applyProofStrategy(Proof proof) {
        keYEnvironment.getProofControl().startAndWaitForAutoMode(proof);
        if (mode == Mode.DEBUG)
            debugger();
    }

    private void applyProofMacro(Proof proof, ProofMacro proofMacro) {
        ProofControl proofControl = keYEnvironment.getProofControl();
        proofControl.runMacro(proof.root(), proofMacro, null);
